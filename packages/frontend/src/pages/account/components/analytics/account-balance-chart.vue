<template>
  <div v-if="chartData.length < 2" class="text-muted-foreground py-16 text-center text-sm">
    Not enough balance history to show a chart yet.
  </div>
  ...
  <div v-else ref="containerRef" class="relative h-80 w-full">
    ...
    >
      <ChartTooltip>
        ...
        <ChartTooltipRow total label="Balance" :value="formatBaseCurrency(tooltip.amount)" />
      </ChartTooltip>
    </div>
  </div>
</template>
...
const props = defineProps<{
  data: endpointsTypes.AccountAnalyticsBalancePoint[];
}>();
...
const tooltip = reactive({ visible: false, x: 0, y: 0, date: '', amount: 0 });
const { updateTooltipPosition } = useChartTooltipPosition({ containerRef, tooltipRef, tooltip });

const chartData = computed(() =>
  props.data
    .map((point) => ({ date: parseISO(point.date).getTime(), amount: point.amount }))
...
const renderChart = () => {
  ...
  const [yMinRaw, yMaxRaw] = d3.extent(chartData.value, (d) => d.amount) as [number, number];
  ...
  const gradientId = `account-balance-gradient-${Math.random().toString(36).slice(2)}`;
  const gradient = svg
    .append('defs')
  ...
  g.append('rect')
    ...
    .on('mousemove', (event: MouseEvent) => {
      ...
      hoverDot.attr('cx', xScale(d.date)).attr('cy', yScale(d.amount));
